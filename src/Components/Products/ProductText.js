...
import useAddToCart from "./../../hook/cart/add-to-cart-hook";

const ProductText = () => {
  const { id } = useParams();
  const [item, images, cat, brand] = useViewProductsDetalis(id);
  const [colorClick, indexColor, addToCartHandel] = useAddToCart(id, item);
